...
@interface MWPhotoBrowser () {
	...
	UIBarButtonItem *_previousButton, *_nextButton, *_actionButton,*_airPlayButton;
...
- (id)init {
    if ((self = [super init])) {
        ...
        airPlayManager = [[AKAirplayManager alloc] init];
...
- (void)viewDidLoad {
    ...
    _airPlayButton = [[UIBarButtonItem alloc]initWithBarButtonSystemItem:UIBarButtonSystemItemOrganize target:self action:@selector(showAirPlayOptions)];
...
- (void)performLayout {
    ...
    [items addObject:_airPlayButton];
...
- (void)handleMWPhotoLoadingDidEndNotification:(NSNotification *)notification {
    ...
    if (page) {
        if ([photo underlyingImage]) {
            if (airPlayManager.connectedDevice) {
                [airPlayManager.connectedDevice sendImage:[photo underlyingImage]];
            }
            // Successful load
            [page displayImage];
...
- (void) manager:(AKAirplayManager *)manager didConnectToDevice:(AKDevice *)device
{
    ...
    id<MWPhoto> currentPicture = [self photoAtIndex:_currentPageIndex];
    if ([currentPicture underlyingImage]) {
        if(manager.connectedDevice)
        {
//            NSLog(@"Sending dummy image");
            
            //        [manager.connectedDevice sendImage:[UIImage imageNamed:@"Evening.png"] ];
            [manager.connectedDevice sendImage:[currentPicture underlyingImage] forceReady:YES];
        }
...
- (void)didStartViewingPageAtIndex:(NSUInteger)index {
    ...
    if ([currentPhoto underlyingImage]) {
        if (airPlayManager.connectedDevice) {
            [airPlayManager.connectedDevice sendImage:[currentPhoto underlyingImage]];
        }
